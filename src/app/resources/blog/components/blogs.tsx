      ...
      <div className="grid grid-cols-1 justify-self-center md:grid-cols-2 gap-8">
        {posts.map((post) => (
          <div key={post.slug} className="bg-white flex flex-col max-w-[590px]">
            {post.thumbnail && (
               <div className="block relative w-full mb-4 group">
                 <Link href={`/resources/blog/${post.slug}`} className="absolute inset-0 z-0" aria-label={post.title} />
                 <Image
                   src={post.thumbnail}
                   alt={post.title}
                   width={590}
                 ...
                 />
                 <div className="absolute left-0 bottom-0 flex items-start justify-between p-5 border-t border-white/30 bg-[rgba(227,227,227,0.30)] backdrop-blur-md w-full z-10 pointer-events-none">
                   <div className="flex flex-col items-start gap-1">
                     <div className="flex flex-wrap gap-1 leading-5 font-semibold text-sm text-white pointer-events-auto">
                       {post.authors?.map((authorName, index) => {
                          const slug = authorName === 'Hasham Ul Haq' ? 'hasham-ul-haq' : authorName === 'Ritwik Jain' ? 'ritwik-jain' : '#';
                          return (
                           <span key={authorName}>
                             <Link href={`/author/${slug}`} className="hover:underline hover:text-white/80 transition-colors">
                               {authorName}
                             </Link>
